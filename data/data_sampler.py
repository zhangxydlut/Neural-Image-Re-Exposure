...
class MTDistIterSampler(Sampler):
    """ Sampler loading data for different tasks in different iterations
    Modified from DistIterSampler
    """

    def __init__(self, dataset, num_replicas=None, rank=None, ratio=100):
        super(MTDistIterSampler, self).__init__(dataset)
        if num_replicas is None:
            if not dist.is_available():
                raise RuntimeError("Requires distributed package to be available")
            num_replicas = dist.get_world_size()
