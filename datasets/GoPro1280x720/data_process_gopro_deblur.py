"""
Data preprocessing script of GoPro.
References:
    eSLNet: https://www.ecva.net/papers/eccv_2020/papers_ECCV/papers/123580154.pdf
            https://github.com/ShinyWang33/eSL-Net#synthetic-dataset
    NEST:   https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/136660649.pdf  # Supplement Page-1
            https://github.com/ChipsAhoyM/NEST

Usage:
1. Install V2E (Please refer to Neural-Image-Re-Exposure/data/tools/v2e/README.md)
2. Put raw video into DATA_ROOT/RAW_DIR (Refer to Neural-Image-Re-Exposure/datasets/GoPro1280x720/VideoRaw/INFO.md)
...
split = 'train'
# Paths
DATA_ROOT = os.path.abspath(f"./Deblur")
# DATA_ROOT = os.path.abspath(f"/media/zxy/AnyPixEF/Dataset/GoPro{W}x{H}/Deblur")
RAW_DIR = os.path.abspath(os.path.join(DATA_ROOT, f"../VideoRaw/{split}"))
...
N_BINS = 36
CLIP_STEP = 11
blur_candidates = [11, 7]
MAX_BLUR = max(blur_candidates)
...
def get_gs_clip(vid, start_index, clip_length, im_size=(1280, 720)):
    """
    Get a clip of video consists of GS frames as the source of data synthesis of a "sample"
    Args:
...
def run_gopro_process():
    ...
    for vid in vid_path_list:
        ...
        while True:
            sample_dir = os.path.join(SAMPLE_DIR, vid_name, "{:06}_{:06}".format(clip_start, clip_start+1+MAX_BLUR-1))

            clip = get_gs_clip(vid, start_index=clip_start, clip_length=1+MAX_BLUR-1, im_size=(W, H))
            if len(clip) < MAX_BLUR or (clip_start + MAX_BLUR) > len(os.listdir(os.path.join(EVENT_DIR, vid_name, 'ev_npz'))):
                break
            ...
            save_gs_frame_with_vg(clip, gs_start=0, blur_factor=11, sample_dir=sample_dir, evnpz_list=evnpz_list)
            # save_gs_frame_with_vg(clip, gs_start=2, blur_factor=7, sample_dir=sample_dir, evnpz_list=evnpz_list)
