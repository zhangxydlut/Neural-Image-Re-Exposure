...
split = 'train'
# Paths
DATA_ROOT = os.path.abspath(f"./VFI-{CLIP_STEP+1}")
...
def run_gopro_process():
    ...
    for vid in vid_path_list:
        ...
        while True:
            ...
            save_gs_only(clip, gs_start=0, blur_factor=1, sample_dir=sample_dir)
            save_gs_only(clip, gs_start=CLIP_STEP, blur_factor=1, sample_dir=sample_dir)
...
def save_gs_only(clip, gs_start, blur_factor, sample_dir):
    ...
    gs_frame = np.stack(densify_clip(clip[gs_start:gs_start+gs_span+(blur_factor-1)], factor=1, mode='replica'), axis=0).mean(0).astype(np.uint8)
