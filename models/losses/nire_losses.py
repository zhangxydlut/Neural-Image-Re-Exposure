...
from models.losses.basicsr_loss import PSNRLoss
...
class NIRELoss(nn.Module):
    def __init__(self):
        super(NIRELoss, self).__init__()
        self.pix_crit_l1 = nn.L1Loss(size_average=True)
        self.pix_crit_mse = nn.MSELoss()
        self.psnr_cirt = PSNRLoss()
        self.ssim_module = SSIM()  # for ssim loss calculation
        self.vgg = VGG19().cuda().eval()  # for perceptive loss

        self.loss_record = dict()

    def featStyleLoss(self, pred, y):

        weights = [1.0 / 32, 1.0 / 16, 1.0 / 8, 1.0 / 4, 1.0]  # weights for feat style loss
        ...
        for i in range(len(x_vgg)):
            f_loss += weights[i] * self.pix_crit_l1(x_vgg[i], y_vgg[i].detach())
            s_loss += weights[i] * self.pix_crit_l1(gram_matrix(x_vgg[i]),
                                                         gram_matrix(y_vgg[i].detach()))
